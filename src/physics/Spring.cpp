...
Spring::Spring(std::shared_ptr<Particle> a, std::shared_ptr<Particle> b, float stiffness)
    : p1(a), p2(b), stiffness(stiffness)
{
    if (!p1 && !p2) return;
    restingLength = (p2->GetPosition() - p1->GetPosition()).length();
    compliance = (1.0f - stiffness) / std::max(stiffness, 1e-6f); // Prevent division by zero
...
void Spring::SolveConstraints(float dt)
{
    ...
    float dlambda = (-C - compliance * lambda) / (wSum + compliance / (dt * dt));
    lambda += dlambda;
